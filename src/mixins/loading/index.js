const load_option = { // 配置初始参数
    lock: true,
    text: "努力加载中。。。",
    spinner: undefined,
    background: "rgba(255, 255, 255, 0.8)",
};
function ld_open(option) { // 方法复写到外部，防止混入过大方法内存
    ...
    try {
        ...
        if(load_o.target){
            if(typeof option.target == "string"){
                ...
                name = `${option.target.nodeName}_${option.target.className}_ld_open_sign_${new Date().getTime()}`;
...
function ld_close(option) { // 方法复写到外部，防止混入过大方法内存
    // option 参数参考  element-ui loading 功能设置的参数 
    ...
    try {
        ...
        if(option.target){
            if(typeof option.target == "string"){
                ...
                name = option.target.attributes.getNamedItem("ld_open_sign").textContent;
...
export default { // 抛出混入对象
    ...
    methods: {
        ld_open(option) { // 配置初始参数， 传入 element-ui loading 功能设置的参数
            // option 参数参考  element-ui loading 功能设置的参数
            // 注意： 这里 target 参数经过修改，可以传入 ref 对象
            ld_open.call(this,option);
        },
        ld_close(option) { // 关闭loading 并清除 loading对象缓存 
            // option 参数参考  element-ui loading 功能设置的参数 
            // 但只需要传递 打开loading时设置的 target、fullscreen、body 三个参数，假如设置时没有传则不需要
